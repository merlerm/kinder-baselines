...
VENV="$KINDER_REGRESSION_ROOT/venvs/bilevel-$LABEL"
if [[ ! -x "$VENV/bin/python" ]]; then
  echo "Missing $VENV. Run setup_bilevel_envs.sh first." >&2
  exit 2
fi

OUTPUT_DIR="$KINDER_REGRESSION_ROOT/results/bilevel/$LABEL/$ENV_CONFIG/seed-$OUTER_SEED"
...
cd "$BASELINES_DIR/kinder-bilevel-planning"
exec "$VENV/bin/python" experiments/run_experiment.py \
  "env=$ENV_CONFIG" \
  "seed=$OUTER_SEED" \
