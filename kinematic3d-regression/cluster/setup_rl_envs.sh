...
# RL batch covers BaseMotion3D (A0/A1) and Transport3D (A0/A1/A2).
for label in A0 A1 A2; do
  revision="$(get_kindergarden_revision "$label")"
  worktree="$WORKTREE_DIR/kindergarden-$label"
  venv="$VENV_DIR/rl-$label"
  ...
  "$venv/bin/python" -m pip install -e "$BASELINES_DIR/kinder-rl"
  "$venv/bin/python" -m pip freeze > "$MANIFEST_DIR/rl-$label-pip-freeze.txt"
  "$venv/bin/python" - <<PY
import kinder
