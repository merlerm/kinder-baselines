...
for label in A0 A1 A2 A3 A4; do
  ...
  # The Transport3D scripted policy imports kinder_models parameterized skills and
  # bilevel_planning trajectory samplers; install the same baseline set as the
  # bilevel venvs plus the policies package.
  "$venv/bin/python" -m pip install \
    -e "$BASELINES_DIR/kinder-ds-policies" \
    -e "$BASELINES_DIR/kinder-imitation-learning" \
    -e "$BASELINES_DIR/kinder-models" \
    -e "$BASELINES_DIR/kinder-bilevel-planning"
  "$venv/bin/python" -m pip freeze > "$MANIFEST_DIR/dsp-$label-pip-freeze.txt"
  "$venv/bin/python" - <<PY
import kinder
